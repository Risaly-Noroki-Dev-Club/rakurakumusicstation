...
📻 RadioServer (Rakuraku Music Station)
一个基于 C++17 开发的高性能、轻量级流媒体广播服务器，采用 Apache License 2.0 协议开源。
本项目专为低延迟音频分发设计，结合了 Linux 原生 epoll 事件驱动模型与现代化的 Web 管理界面。它能将本地媒体目录转化为一个实时流媒体电台，支持多客户端同步收听及远程文件管理。
✨ 功能特性
 * 双引擎架构：
   * 核心流引擎：基于 epoll 的非阻塞 I/O，支持上千路并发连接，极低 CPU 占用。
   * Web 管理引擎：基于 Crow 框架，提供交互式 UI 与文件上传功能。
 * 实时流化：调用 ffmpeg 动态转码，确保音频以恒定的比特率传输，适配各类播放器。
 * 自动化环境检测：内置构建脚本，自动检查依赖项（g++、ffmpeg、openssl）并优化编译产物。
 * 优雅停机：完善的信号处理机制，确保在服务器关闭时正确释放 Socket 和文件句柄。
🛠️ 技术栈
 * 内核：C++17 标准
 * 底层 I/O：Linux epoll, POSIX Threads, Pipes
...
# Ubuntu/Debian 示例
sudo apt update
sudo apt install build-essential libssl-dev ffmpeg

注意：请确保 crow_all.h 位于项目根目录。

4. 启动与访问
 * 进入构建目录：cd release
 * 运行服务器：./radioserver_release
 * Web 管理平台: http://localhost:2240 (用于点歌、查看状态、上传文件)
 * 音频流地址: http://localhost:2241 (可直接在浏览器或播放器打开)
📂 项目结构
 * StreamServer: 负责维护客户端 Socket 队列，通过 epoll 实现高效分发。
 * AudioPlayer: 负责读取音频，通过匿名管道将解码流推送到缓冲区。
 * WebServer: 基于 Crow 提供的 Web API 和 UI 交互。
 * media/: 存放音频文件的默认目录。
📜 开源协议
本项目采用 Apache License 2.0 协议开源。详情请参阅项目中的 LICENSE 文件。
🤝 鸣谢 (Acknowledgments)
